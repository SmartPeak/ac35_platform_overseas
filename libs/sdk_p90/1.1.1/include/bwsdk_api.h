...
/**
 * @brief 获取指定utf8数据的宽度像素
 * @author liudeyu@basewin.com
 * @param utf8 [in] 字符串数据
 * @retval >0 数据宽度长度
 * @retval <0 失败
 * @pre 无
 */
int ScrGetUtf8StrPixsWidth(unsigned char* utf8);
...
/**
 * @brief set the format of the screen align mode
 * @brief 设置显示对齐方式(左对齐|右对齐)
 * @param alignmode [in] align mode (scr_align_mode)
 */
void ScrSetAlignMode(scr_align_mode alignmode);

/**
 * @brief get the format of the screen align mode
 * @brief 获取显示对齐方式
 * @retval 显示对齐方式(左对齐|右对齐) renturn value: align mode for the screen
 */
scr_align_mode ScrGetAlignMode(void);

/**
 * @brief 设置显示换行模式(自动换行|不换行)
 * @author liudeyu@basewin.com
 */
void ScrSetWrapMode(scr_wrap_mode wrapmode);

/**
 * @brief 获取显示换行方式
 * @author liudeyu@basewin.com
 */
scr_wrap_mode ScrGetWrapMode(void);

/**
 * @brief 设置显示换行行间距(自动换行模式)
 * @author liudeyu@basewin.com
 */
void ScrSetLineSpace(unsigned char linespace);

/**
 * @brief 获取显示换行行间距(自动换行模式)
 * @author liudeyu@basewin.com
 */
unsigned char ScrGetLineSpace(void);
...
#define WIFI_MODE_STATION_SOFTAP    (3)     ///< 

// wifi scan option
#define SCAN_OPT_ECN                (0x01)  ///< 
#define SCAN_OPT_SSID               (0x02)  ///< 
#define SCAN_OPT_RSSI               (0x04)  ///< 
#define SCAN_OPT_MAC                (0x08)  ///< 
#define SCAN_OPT_CH                 (0x10)  ///< 
#define SCAN_OPT_FREQ_OFFSET        (0x20)  ///< 
#define SCAN_OPT_FREQ_CALIBRATION   (0x40)  ///< 
...
#define INFO_SSID_SIZE_MAX  (32) ///< ssid最大32个字符 
#define INFO_MAC_SIZE       (17) ///< mac为固定17字节
...
typedef struct {
    char ssid[INFO_SSID_SIZE_MAX+1];  ///<
    char mac[INFO_MAC_SIZE+1];        ///<
    unsigned char ch;                 ///<
    int rssi;                         ///<
    int freq_offset;                  ///<
...
typedef enum {
    WLAN_DPSLP_DISALBE = 0, ///< WLAN deep-sleep disable
    WLAN_DPSLP_ENABLE,      ///< WLAN deep-sleep enable
} WLAN_DPSLP_E;

/**
 * @brief 搜索WIFI并获取WIFI列表 
 * @brief Search WIFI and get WIFI list
 * @param info [out] WIFI列表缓冲 WIFI list buffer
 * @param info_len [in]  WIFI列表缓冲最大值 Maximum WIFI list buffer size
                    [out] 获取到的WIFI列表数量 Size of WIFI list got
 * @retval 0 Success return 0 for success
 * @retval !0 Fail return !0 for fail
 */
int BW_Wlan_GetWifiList(WIFI_INFO_T* info, unsigned int* info_len);
...
/**
 * @brief Get setting of WLAN deep-sleep enable
 * @retval  0: WLAN deep-sleep is disabled
 * @retval !0: WLAN deep-sleep is enabled
 */
int WLAN_IsDeepSleepEnable(void);

/**
 * @brief Set WLAN deep-sleep enable/disable
 * @param [in] enable: 
 *                  0: WLAN deep-sleep is disable
 *                  1: WLAN deep-sleep is enable
 * @retval none
 */
void WLAN_DeepSleepEnable(int enable);
...
typedef enum
{
	PRINT_FONT_SIZE_NORMAL 	= 0,   ///< NORMAL SIZE
    PRINT_FONT_SIZE_SMALL 	= 1,   ///< SMALL SIZE
    PRINT_FONT_SIZE_LARGE  	= 2,   ///< BIG SIZE
} fontsize_t;
 ...
 */
typedef enum {
    LEFT_T_RIGHT = 0,      ///<from left to right
    RIGHT_T_LEFT,		   ///<from right to left
    COMPOSE_MODE_MAX
} printcompose_t;
...
typedef struct
{
    ...
    printcompose_t printCompose;///< Compose 排版方式 
